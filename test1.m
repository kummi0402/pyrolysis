...
numtime = 20;
T_surf = zeros(N, numtime);
t_surf = zeros(N, numtime);
x_surf = zeros(N, numtime);
%%%%%%%%%%%%%%%
...
desired_times     = 0:(25/(numtime-1)):25;    %numtime = 20;
desired_locations = 0:(0.275/(N-1)):0.275;    %N = 26

recorded_times = zeros(length(desired_temperatures), length(probe_locations));    %OLD

%recorded_temperature_forDistance = zeros(N, numtime);
recorded_temperature_forDistance = zeros(length(desired_locations), length(desired_times));
%recorded_temperature_forTime = zeros(N, numtime);
recorded_temperature_forTime = zeros(length(desired_locations), length(desired_times));
%recorded_temperature = zeros(N, numtime);
%recorded_temperature = zeros(length(desired_locations), length(desired_times));
...
%desired_locations = 0:(0.275/(N-1)):0.275;    %N = 26
    ...
    %% Temperature calculated for distance
    for i = 1 : N
        %x_this = (i - M) * dx + 0.05;
        x_this = (i) * dx;
        x_next = x_this + dx;
        ...
        for j = 1 : length(desired_locations)
            if x_this <= desired_locations(j) && x_next >= desired_locations(j)    %if x_this <= probe_locations(j) && x_next >= probe_locations(j)
                for k = 1 : length(desired_times)
                    if (recorded_temperature_forDistance(j, k) == 0)
                      % constant interpolation
                      recorded_temperature_forDistance(j, k) = T(i);
                    end
                end
            end
        end
    ...
    %% Temperature calculated for time
    for i = 1 : numtime
        t_this = (i) * delt;
        t_next = t_this + delt;
        ...
        % x_M --- x_M+1 --(p1)-- x_M+2 --- x_M+3 --(p2)-- x_M+4 ...
        for k = 1 : length(desired_times)
            if t_this <= desired_times(k) && t_next >= desired_times(k)    %if x_this <= probe_locations(j) && x_next >= probe_locations(j)
                for j = 1 : length(desired_locations)
                    if (recorded_temperature_forTime(j, k) == 0)
                      % constant interpolation
                      recorded_temperature_forTime(j, k) = T(i);
                    end
                end
            end
        end
...
surf (desired_times, desired_locations, recorded_temperature_forDistance)
